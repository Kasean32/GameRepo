...
-- every game made by sony where the condition that is new in alphabetical order

CREATE OR REPLACE view vPlaystation AS 
SELECT * FROM product p where p.platform = 'sony'
AND p.condition = 'New'
ORDER BY p.name;

-- every customer that only buys games with 15 percent discount or higher
CREATE OR REPLACE view vCustomerDis AS
SELECT customer.memID, order.order_ID, customer.discount
From order 
INNER Join Customer ON customer.memID=order.order_ID;

-- List all games that are second most expensive games for playstation.
CREATE OR REPLACE view vGame2ndExpensive AS
SELECT * FROM product p1, game g1, product p2, game g2,product p3, game g3 WHERE g1.product_id = p1.id AND g2.product_id = p2.id AND g3.product_id = p3.id 
AND NOT EXISTS (
    p2.price > p1.price
    where not exists (
        p2.price > p3.price
...
--List all employees that have placed an order for all platforms
--CREATE OR REPLACE view vEmployeeSold AS
--SELECT * FROM 
